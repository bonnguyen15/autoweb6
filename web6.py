...
class SPConfig():
    ...
    def __init__(self, filename, sp, spApi, cols):
        self.filename = filename ; self.sp = sp; self.spApi = spApi; self.cols = cols
    ...
    def configApi(self):
        os.system(f"git clone https://ptecdgn:{self.passgit}@bitbucket.org/diginetvn/{arr[2][self.cols[0]]} {dirsource}\{arr[2][self.cols[0]]}")
...
for sp in arr[0]:
        ...
        if arr[1][cols] != "":
            for i in range(int(arr[1][cols[0]])):
                filename = sp + "." + str(i)
                spConfig = SPConfig(filename, sp, spApi, cols)
                spConfig.createFolder()
                spConfig.configApi()
